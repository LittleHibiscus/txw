        ...
        <div class="zss-panel">
          ...
          <div class="zss-panel-body">
            ...
            <div class="zss-date-week" v-show="select=='date'">
              <div class="zss-week" v-for="(item,index) in weekArray" :key="index">{{item}}</div>
            </div>
            <!-- /周标题 -->
            <!-- 日期-->
            <div class="zss-date-box" v-show="select=='date'">
              <div
                class="zss-date"
                :class="{'zss-date-now':isNow(item),'zss-month-now':isMonth(item)}"
...
export default {
  ...
  data() {
    return {
      currentValue: momnet().format("YYYY-MM-DD"), //当前值
      zIndex: -1,
      isShow: false,
      select: "date",
